...
sock = socket(AF_INET, SOCK_STREAM)     # make a TCP socket object
sock.bind((myHost, myPort))             # bind to server port number
sock.listen(5)                          # allow 5 pending connects
...
def dispatcher():                       # listen until process killed
    while True:                         # wait for next connection
        connection, address = sock.accept() # pass to process for service
        ...
        if childPid == 0:               # if in child process: handle
            ...
            activeChildren.append(childPid) # else: go accept next connect
                                            # add to active child pid list
